...
const openAddModal = () => {
  newAssignment.officerEmail = officersList.value.length > 0 ? officersList.value[0].email : '';
  newAssignment.cameraID = availableCameras.value.length > 0 ? availableCameras.value[0].cameraID : '';
  formErrors.officerEmail = '';
  formErrors.cameraID = '';
...
const handleAddAssignment = async () => {
  ...
  const exists = assignments.value.some(a => a.cameraID === newAssignment.cameraID);
  if (exists) {
...
const handleBulkDelete = async () => {
  ...
  if (!confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบ ${selectedIds.value.size} รายการ?`)) {
    return;
  }
...
<template>
  <div class="py-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
      <div>
        <h2 class="text-3xl font-bold text-base-content mb-2">จัดการ Assignment</h2>
      ...
      <div class="flex gap-2">
        <button @click="fetchCameras(); fetchOfficers(); fetchAssignments();" class="btn btn-ghost gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      <div class="stats shadow bg-base-100">
        <div class="stat">
          ...
          <div class="stat-value text-warning">{{ availableCameras.length }}</div>
      ...
      <div class="stats shadow bg-base-100">
        <div class="stat">
          ...
          <div class="stat-value text-accent">{{ topOfficer ? topOfficer.count : 0 }}</div>
          <div class="stat-desc">{{ topOfficer ? getOfficerName(topOfficer.email) : '-' }}</div>
        </div>
      </div>
    </div>
      ...
      <div class="card-body p-4">
        <div class="flex flex-col md:flex-row gap-4">
          ...
          <div class="form-control">
            <select v-model="filterOfficer" class="select select-bordered min-w-[200px]">
              ...
              <option 
                ...
                :value="officer.email"
              >
                {{ officer.name }} ({{ assignmentsByOfficer[officer.email] || 0 }})
              </option>
            </select>
          </div>

          <div v-if="selectedIds.size > 0" class="flex gap-2">
            <button @click="handleBulkDelete" class="btn btn-error btn-sm gap-2">
    ...
    <!-- Table -->
    <div v-else-if="filteredAssignments.length > 0" class="card bg-base-100 shadow-lg overflow-hidden">
        ...
        <table class="table table-zebra">
          ...
          <tbody>
            ...
            >
              ...
              <td>
                <span class="badge badge-primary">{{ item.cameraID }}</span>
              </td>
              <td>
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="font-bold">{{ getCameraInfo(item.cameraID)?.cameraName || item.cameraID }}</div>
                    <div v-if="getCameraInfo(item.cameraID)?.latitude" class="text-xs text-base-content/70">
                      📍 {{ getCameraInfo(item.cameraID).latitude.toFixed(4) }}, {{ getCameraInfo(item.cameraID).longitude.toFixed(4) }}
                    </div>
                  </div>
                </div>
              </td>
                ...
                <div class="flex items-center gap-2">
                  <div class="avatar placeholder">
                    <div class="bg-neutral text-neutral-content rounded-full w-8">
                      <span class="text-xs">{{ getOfficerName(item.officerEmail)[0] }}</span>
                    </div>
                  </div>
                  <span class="text-sm">{{ getOfficerName(item.officerEmail) }}</span>
                </div>
              </td>
              <td class="text-right">
    ...
    <div v-else class="text-center py-20">
      ...
      <p class="text-base-content/70 mb-4">
        {{ searchQuery ? 'ไม่พบผลลัพธ์ที่ตรงกับคำค้นหา' : 'ยังไม่มีการมอบหมายกล้องในระบบ' }}
      </p>
      <div v-if="!searchQuery && cameras.length === 0" class="alert alert-warning max-w-md mx-auto">
        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
        <span>กรุณาเพิ่มกล้องก่อนสร้าง Assignment</span>
      </div>
      <div v-else-if="!searchQuery && officersList.length === 0" class="alert alert-warning max-w-md mx-auto">
        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
      ...
      <button 
        v-else-if="!searchQuery && availableCameras.length > 0 && officersList.length > 0" 
        @click="openAddModal" 
    ...
    <dialog id="add_assignment_modal" class="modal">
      <div class="modal-box max-w-lg">
        ...
        <form @submit.prevent="handleAddAssignment" class="space-y-4">
          ...
          <div class="form-control">
            <label class="label">
              <span class="label-text font-semibold">เจ้าหน้าที่รับผิดชอบ <span class="text-error">*</span></span>
            </label>
            <select
            ...
            <label v-if="formErrors.officerEmail" class="label">
              <span class="label-text-alt text-error">{{ formErrors.officerEmail }}</span>
            </label>
          </div>
          ...
          <div class="form-control">
            <label class="label">
              <span class="label-text font-semibold">เลือกกล้อง <span class="text-error">*</span></span>
            </label>
            <select
            ...
            <label v-else class="label">
              <span class="label-text-alt">เฉพาะกล้องที่ยังไม่ถูกมอบหมาย ({{ availableCameras.length }} ตัว)</span>
            </label>
          </div>
          ...
          <div v-if="newAssignment.cameraID" class="alert alert-info">
            ...
            <div class="text-sm">
              <strong>{{ getCameraInfo(newAssignment.cameraID)?.cameraName }}</strong>
              <div v-if="getCameraInfo(newAssignment.cameraID)?.latitude" class="text-xs">
          ...
          <div class="modal-action">
            <button type="button" class="btn btn-ghost" onclick="add_assignment_modal.close()">
              ยกเลิก
            </button>
        ...
        >
          ...
          <div class="form-control">
            ...
            <div class="alert">
              ...
              <div>
                <strong>{{ editingAssignment.cameraID }}</strong> - {{ getCameraInfo(editingAssignment.cameraID)?.cameraName }}
              </div>
            </div>
            <label class="label">
          ...
          <!-- Actions -->
          <div class="modal-action">
            <button type="button" class="btn btn-ghost" onclick="edit_modal.close()">
              ยกเลิก
            </button>
