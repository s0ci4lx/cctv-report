...
<template>
  <div class="navbar bg-base-100 shadow-md mb-4 px-4 md:px-8 lg:px-12">
    <div class="flex-1 flex items-center">
      <!-- Mobile Hamburger Menu with Title (แสดงเฉพาะใน mobile เมื่อ login) -->
      <div v-if="isLoggedIn" class="dropdown md:hidden">
        <div tabindex="0" role="button" class="btn btn-ghost text-lg gap-2">
          <svg
          ...
          <span>รายงานกล้องวงจรปิด สภ.เทพา</span>
        </div>
        <ul
          tabindex="0"
      ...
      <!-- Desktop Title (ไม่มี dropdown) -->
      <a v-if="isLoggedIn" class="btn btn-ghost text-lg md:text-xl gap-2 hidden md:inline-flex">
        <svg
          xmlns="http://www.w3.org/2000/svg"
      ...
      <a v-if="!isLoggedIn" class="btn btn-ghost text-lg md:text-xl gap-2">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-5 w-5 md:h-6 md:w-6"
          fill="none"
